...
//the signal, and the answer is how long we must wait for the slowest one.so max
    int networkDelayTime(vector<vector<int>>& times, int n, int k) {
        vector<vector<pair<int,int>>> adj(n+1);  //0 indfex will be unused or we can use unordered map
        for(auto& ad:times){
            adj[ad[0]].push_back({ad[1],ad[2]});
        }
        vector<int> result(n+1,INT_MAX);//0 index will be used
        priority_queue<pair<int,int>,vector<pair<int,int>>,greater<pair<int,int>>> pq;//min heap source,time
        result[k]=0;//time to travel from source to source =0
        pq.push({0,k});//time node
        while(!pq.empty()){
            auto [time,node]=pq.top();
            pq.pop();
             if(time>result[node]) continue ;//which is true, so we skip this outdated entry.
            for(auto& [nbr,t]:adj[node]){
                if(result[nbr]>result[node]+t){
                    result[nbr]=result[node]+t;
